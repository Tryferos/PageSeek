...
export const BookLinks = ({links}: Props) => {
  ...
  return (
    <div className="flex flex-col gap-y-2 mb-2 mt-1">
      <p className="text-sm font-wotfardMd text-secondary underline underline-offset-2">
        Read online
      </p>
      <ul className="flex scrollbar flex-col gap-y-2 max-h-[10vw] min-h-[90px] overflow-y-auto w-full overflow-x-hidden">
        {links.map((link, i) => (
          <Link
            key={i}
            href={link.url}
            target="_blank"
            className="flex gap-x-0 w-[50%] focus:outline-none items-center focus:scale-101 focus:translate-x-1 focus:underline hover:underline hover:scale-101 hover:translate-x-1 text-secondary transition-transform group">
